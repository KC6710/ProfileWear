...
class Customer extends \Opencart\System\Engine\Controller {
	...
	protected function getList(): string {
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($customer_total) ? (($page - 1) * 200) + 1 : 0, ((($page - 1) * 200) > ($customer_total - 200)) ? $customer_total : ((($page - 1) * 200) + 200), $customer_total, ceil($customer_total / 200));
	...
	public function updateNewDetails(): void {
		$this->load->language('customer/customer');
		...
		if (!$json) {
			if (!$this->request->post['customer_id']) {
				...
				$this->model_customer_customer->updateNewDetails($this->request->post['customer_id'], $this->request->post);
			}
	...
	public function getIp(): string {
		...
		foreach ($results as $result) {
			$store_info = $this->model_setting_store->getStore($result['customer_id']);

			if ($store_info) {
				$store = $store_info['name'];
			} elseif (!$result['customer_id']) {
	...
	public function autocomplete(): void {
		...
		if (isset($this->request->get['filter_search'])) {
			...
			foreach ($results as $result) {
				...
				if($company != ''){
					$customerstr ='[#'.$result['customer_id'].' | '. $result['firstname'] .' '. $result['lastname'] . ' | '. $company .' | '. $result['email'].']' ;
				}else{
					$customerstr ='[#'.$result['customer_id'].' | '. $result['firstname'] .' '. $result['lastname'] .' | '. $result['email'].']' ;
				}
				$json[] = [
					'customer_id'       => $result['customer_id'],
					'customer_group_id' => $result['customer_group_id'],
					'name'              => strip_tags(html_entity_decode($customerstr)),
					'customer_group'    => $result['customer_group'],
	...
	public function getAddressesByCustomerId(): void {
		$json = [];
		if (isset($this->request->get['customer_id'])) {
			$customer_id = (int)$this->request->get['customer_id'];
		} else {
		...
		$address_info = $this->model_customer_customer->getAddresses($customer_id);
	...
	public function getCustomer(): void {
		...
		$this->load->model('customer/customer');
		// echo $customer_id; die;
		$results = $this->model_customer_customer->getCustomer($customer_id);
		...
		if (!$json) {
				$json[] = [
					...
					'custom_field'      => json_decode($results['custom_field'], true),
					'address'           => $this->model_customer_customer->getAddresses($results['customer_id'])
				];
		}
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function updateAddressById(){
		$this->load->model('customer/customer');
		$this->model_customer_customer->updateNewAddress($this->request->post['address_id'], $this->request->post);
	}
	public function import(){
		$this->load->model('customer/customer');
		$this->model_customer_customer->import();
	}
}
