...
class Category extends \Opencart\System\Engine\Controller {
	...
	protected function getList(): string {
		...
		foreach ($results as $result) {
			$seo_keyword = $this->model_catalog_category->getSeoUrls($result['category_id']);
			$parent = '';
			if($result['parent_id']){
				$parent = $this->model_catalog_category->getCategory($result['parent_id']); 
			}
			$data['categories'][] = [
				...
				'seo_url'     => count($seo_keyword) > 0 ? HTTPS_CATALOG . ltrim($seo_keyword[0][$this->config->get('config_language_id')],"/"): '' ,
				'seo_keyword' => count($seo_keyword) > 0 ? $seo_keyword[0][$this->config->get('config_language_id')]: '',
		...
		$data['sort_id'] = $this->url->link('catalog/category.list', 'user_token=' . $this->session->data['user_token'] . '&sort=category_id' . $url);
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($category_total) ? (($page - 1) * $category_total) + 1 : 0, ((($page - 1) * $category_total) > ($category_total - $category_total)) ? $category_total : ((($page - 1) * $category_total) + $category_total), $category_total, ceil($category_total / $category_total));
	...
	public function form(): void {
		...
		if (!empty($category_info)) {
			$data['category_color'] = $category_info['category_color'];
		} else {
			$data['category_color'] = '';
		}
	...
	public function import(){
		$this->load->model('catalog/category');
		$this->model_catalog_category->import();
	}
}
