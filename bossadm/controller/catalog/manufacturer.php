...
class Manufacturer extends \Opencart\System\Engine\Controller {
	...
	public function form(): void {
		...
		if (!empty($manufacturer_info)) {
			$data['manufacturer_description'] = $this->model_catalog_manufacturer->getManufacturerDescripton($manufacturer_info['manufacturer_id']);
			if($data['manufacturer_description']['background_image']){
				if (is_file(DIR_IMAGE . html_entity_decode($data['manufacturer_description']['background_image'], ENT_QUOTES, 'UTF-8'))) {
					$data['background_thumb'] = $this->model_tool_image->resize(html_entity_decode($data['manufacturer_description']['background_image'], ENT_QUOTES, 'UTF-8'), 100, 100);
				} else {
					$data['background_thumb'] = $data['placeholder'];
				}
			}
		} else {
	...
	// 	$this->load->model('catalog/manufacturer');
	// 	$this->model_catalog_manufacturer->import();
	// }
}
