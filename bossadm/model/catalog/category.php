...
class Category extends \Opencart\System\Engine\Model {
	public function addCategory(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "category` SET `parent_id` = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `category_color` = '" . $data['category_color'] . "', `text_color` = '" . $data['category_text_color'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `date_modified` = NOW(), `date_added` = NOW()");
	...
	public function editCategory(int $category_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "category` SET `parent_id` = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `category_color` = '" . $data['category_color'] . "', `text_color` = '" . $data['category_text_color'] . "', `status` = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', `date_modified` = NOW() WHERE `category_id` = '" . (int)$category_id . "'");
	...
	// 	// $query = $this->db->query('SELECT * FROM `'. DB_PREFIX .'category_pw3`');
	// 	// $i=0;
	// 	// foreach($query->rows as $row){
	// 	// 	$this->db->query('INSERT INTO `'. DB_PREFIX .'category` SET `category_id`="'.$row['category_id'].'", `image`="'.$row['image'].'", `parent_id`="'.$row['parent_id'].'", `top`="'.$row['top'].'", `column`="'.$row['column'].'", `sort_order`="'.$row['sort_order'].'", `status`="'.$row['status'].'", `extra_parents`="'.$row['extra_parents'].'"' );
	// 	// 	$i++;
	// 	// }
	// 	// if($i == count($query->rows)){
	// 	// 	echo "done"; die;
	// 	// }
	// 	// $query_desc = $this->db->query('SELECT * FROM `'. DB_PREFIX .'category_description_pw3`');
	// 	// $i=0;
	// 	// foreach($query_desc->rows as $row){
	// 	// 	$this->db->query('INSERT INTO `'. DB_PREFIX .'category_description` SET `category_id`="'.$row['category_id'].'", `language_id`="'.$row['language_id'].'", `name`="'.$row['name'].'", `title_tag`="'.$row['title_tag'].'", `alt_tag`="'.$row['alt_tag'].'", `description`="'.$row['description'].'", `meta_title`="'.$row['meta_title'].'", `meta_description`="'.$row['meta_description'].'", `meta_keyword`="'.$row['meta_keyword'].'", `h1`="'.$row['h1'].'", `h2`="'.$row['h2'].'"' );
	// 	// 	$i++;
	// 	// }
	// 	// if($i == count($query_desc->rows)){
	// 	// 	echo "done"; die;
	// 	// }		
	// 	// $query_path = $this->db->query('SELECT * FROM `'. DB_PREFIX .'category_path_pw3`');
	// 	// $i=0;
	// 	// foreach($query_path->rows as $row){
	// 	// 	$this->db->query('INSERT INTO `'. DB_PREFIX .'category_path` SET `category_id`="'.$row['category_id'].'", `path_id`="'.$row['path_id'].'", `level`="'.$row['level'].'"' );
	// 	// 	$i++;
	// 	// }
	// 	// if($i == count($query_path->rows)){
	// 	// 	echo "done"; die;
	// 	// }
	// 	// $query_layout = $this->db->query('SELECT * FROM `'. DB_PREFIX .'category_to_layout_pw3`');
	// 	// $i=0;
	// 	// foreach($query_layout->rows as $row){
	// 	// 	$this->db->query('INSERT INTO `'. DB_PREFIX .'category_to_layout` SET `category_id`="'.$row['category_id'].'", `store_id`="'.$row['store_id'].'", `layout_id`="'.$row['layout_id'].'"' );
	// 	// 	$i++;
	// 	// }
	// 	// if($i == count($query_layout->rows)){
	// 	// 	echo "done"; die;
	// 	// }
	// 	$query_store = $this->db->query('SELECT * FROM `'. DB_PREFIX .'category_to_store_pw3`');
	// 	$i=0;
	// 	foreach($query_store->rows as $row){
	// 		$this->db->query('INSERT INTO `'. DB_PREFIX .'category_to_store` SET `category_id`="'.$row['category_id'].'", `store_id`="'.$row['store_id'].'"' );
	// 		$i++;
	// 	}
