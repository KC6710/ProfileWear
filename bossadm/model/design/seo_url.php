...
class SeoUrl extends \Opencart\System\Engine\Model {
	...
	public function import(){
		$query = $this->db->query("SELECT * FROM `". DB_PREFIX ."seo_url` WHERE `key` = 'product_id'");
		$i= 0;
		foreach($query->rows as $row){
			if($row["key"] == 'product_id'){
				$value_array = explode("_", $row["value"]);
				// echo "<pre>"; print_r($value_array);
				// echo $row['seo_url_id']."<br>"; 
				if(count($value_array) > 1){
					$this->db->query("UPDATE `". DB_PREFIX ."seo_url` SET `value` = '".$value_array[count($value_array)-1]."' WHERE `seo_url_id` = '".$row['seo_url_id']."'");
				}
				// echo "INSERT INTO `". DB_PREFIX ."seo_url` SET 
				// `seo_url_id` ='".$row['seo_url_id']."',
				// `store_id` ='".$row['store_id']."',
				// `language_id` ='".$row['language_id']."',
				// `key` ='".$key_array[0]."',
				// `value` ='".$key_array[1]."',
				// `keyword` ='".$row['keyword']."'"."<br>";
				// if($this->db->query("INSERT INTO `". DB_PREFIX ."seo_url` SET 
				// `seo_url_id` ='".$row['seo_url_id']."',
				// `store_id` ='".$row['store_id']."',
				// `language_id` ='".$row['language_id']."',
				// `key` ='".$key_array[0]."',
		 ...
		 die;
		if(count($query->rows) == $i){
			echo "done"; die;
		}else{
			echo "lkserhgiosr"; die;
		}
	}
}
