var autocomplete = {
    ...
    $(this.$refs['input']).autocomplete({
        'source': function (request, response) {
            $.ajax({
                url: 'index.php?route=codevoc/b2bmanager_inventoryprint.spcountautocomplete&user_token='+_self.token+'&filter_name=' + encodeURIComponent(request),
                dataType: 'json',
...
var addArticleComponent = {
	...
	methods:{
		...
		save:function(event){
			...
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_add_article&user_token='+this.token,result,{
				emulateJSON : true,
...
var editComponent = {
	...
	watch:{
		show:function(newShow){
			if(newShow == true){
				this.currentItem.number = this.item.number;
				this.currentItem.client = this.item.client;
	...
	methods:{
		...
		save:function(){
			...
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_update_article&user_token='+this.token,object,{
				emulateJSON : true,
			}).then(function(response){
				if(response.body.result.success){
					this.isSuccess = true;
		...
		autoCompleteChanged:function(event){
			this.currentItem.client = event.value;
		}
	}
};
...
var addToListComponent = {
	...
	methods:{
		...
		save:function(){
			...
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_add_to_list&user_token='+this.token,object,{
				emulateJSON : true,
			}).then(function(response){
				if(response.body.result.success){
					this.isSuccess = true;
...
var addToListTableComponent = {
	...
	methods:{
		...
		deleteItem:function(item,event){
			...
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_delete_from_list&user_token='+this.token,object,{
				emulateJSON : true,
			}).then(function(response){
				if(response.body.result.success){
					this.isSuccess = true;
		...
		deleteAll:function(event){
			event.preventDefault();
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_delete_all_from_list&user_token='+this.token,{
				emulateJSON : true,
			}).then(function(response){
				if(response.body.result.success){
					this.isSuccess = true;
		...
		openPrint:function(){
			var url = 'index.php?route=codevoc/b2bmanager_inventoryprint.print_view&user_token='+this.token;
			window.open(url,"","height=500,width=800");
		},
		collectAndClear:function(){
			event.preventDefault();
			this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_collect_and_clear&user_token='+this.token,{
				emulateJSON : true,
			}).then(function(response){
				if(response.body.result.success){
					this.isSuccess = true;
...
var app = new Vue({
	...
	methods:{
		getData : function(){
			this.$http.get('index.php?route=codevoc/b2bmanager_inventoryprint.api_get_list&user_token='+this.token).then(function(response){
				// success
				this.items = response.body.items;
				this.addToListItems = response.body.add_to_list_items_print;
			},function(response){
				// error
			});
		},
			...
			if(confirm("Are you sure to delete?")){
				...
				if(id){
				  ...
				  formData.append('id', id);
					this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_delete_item&user_token='+this.token,formData).then(function(response){
						// success
		...
		copyItem:function(item,event){
			...
			if(confirm("Are you sure to copy?")){
				...
				if(id){
				  ...
				  // append string
				  formData.append('id', id);
					this.$http.post('index.php?route=codevoc/b2bmanager_inventoryprint.api_copy_item&user_token='+this.token,formData).then(function(response){
						// success
						alert(response.body.result.success);
						this.getData();
					},function(response){
		...
		updateAddToList:function(){
			this.$http.get('index.php?route=codevoc/b2bmanager_inventoryprint.api_get_add_to_list&user_token='+this.token).then(function(response){
				// success
				this.addToListItems = response.body.add_to_list_items_print;
			},function(response){
				// error
	...
	computed:{
		filteredItems: function(){
			...
			return this.items.filter(function (item) {
				return item.number.toLowerCase().indexOf(self.filter_number.toLowerCase()) !== -1 && item.client.toLowerCase().indexOf(self.filter_client.toLowerCase()) !== -1 && item.name.toLowerCase().indexOf(self.filter_name.toLowerCase()) !== -1 && item.color.toLowerCase().indexOf(self.filter_color.toLowerCase()) !== -1 && item.type.toLowerCase().indexOf(self.filter_type.toLowerCase()) !== -1 && item.category.toLowerCase().indexOf(self.filter_category.toLowerCase()) !== -1 && item.quantity.toLowerCase().indexOf(self.filter_quantity.toLowerCase()) !== -1 && item.location.toLowerCase().indexOf(self.filter_location.toLowerCase()) !== -1
			})
			/*
