...
class OpencartSveaCheckoutConfigTest implements \Svea\WebPay\Config\ConfigurationProvider
{
    ...
    public function getCheckoutMerchantId($country = NULL)
    {
        ...
        else
        {
            ...
            if ($this->config->model_setting_setting->getValue($this->moduleString . 'sco_checkout_test_merchant_id_' . strtolower($country)) !== NULL)
            {
                return $this->config->model_setting_setting->getValue($this->moduleString . 'sco_checkout_test_merchant_id_' . strtolower($country));
            }
            else
            {
                throw new Exception('Could not fetch Merchant Id. CountryCode: ' . $country . ' Environment: Stage');
    ...
    public function getCheckoutSecret($country = NULL)
    {
        ...
        {
            $this->config->load->model('setting/setting');
            if ($this->config->model_setting_setting->getValue($this->moduleString . 'sco_checkout_test_secret_word_' . strtolower($country)) !== NULL)
            {
                return $this->config->model_setting_setting->getValue($this->moduleString . 'sco_checkout_test_secret_word_' . strtolower($country));
            }
            else
            {
                throw new Exception('Could not fetch secret word. CountryCode: ' . $country . ' Environment: Stage');
