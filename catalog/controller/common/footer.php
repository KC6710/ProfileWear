	...
	public function index(): string {
		...
		if ($this->config->get('config_customer_online')) {
			...
			$this->model_tool_online->addOnline($ip, $this->customer->getId(), $url, $referer);
		                ...
		                /* PW Blog post */
						$url    = 'https://profilewear.se/blogg/feed/';
						$xmlfile   = file_get_contents($url);
						$invalid_characters = '/[^\x9\xa\x20-\xD7FF\xE000-\xFFFD]/';
						$xmlfile = preg_replace($invalid_characters, '', $xmlfile);
						// $xmlfile  = file_get_contents('https://profilewear.se/blogg/feed/');
						...
						foreach ($wpfile->item AS $item) {
							...
							$s = strip_tags(substr($item->description, 0, 150));
							

							$relativeLink   = str_replace($url, '', $item->link);
								$data['blogpost'][] = array(
									'title'     => $item->title,
									'href'      => '/blogg/'.$relativeLink,
									'thumbnail' => $item->thumbnail,
									'description' => substr($s, 0, strrpos($s, ' ')) . '... Läs mer'
								);
								if (++$i == 4) break;
						}
						/* PW Blog post */
