...
class Order extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$order_total = $this->model_account_order->getTotalOrders($search);

		$results = $this->model_account_order->getOrders(($page - 1) * $limit, $limit,$search);
		...
		foreach ($results as $result) {
			...
			foreach ($products as $product) {
				...
				if (is_file(DIR_IMAGE . html_entity_decode($product['image'], ENT_QUOTES, 'UTF-8'))) {
					$product_image = $this->model_tool_image->resize(html_entity_decode($product['image'], ENT_QUOTES, 'UTF-8'), $this->config->get('config_image_additional_width'), $this->config->get('config_image_additional_height'));
				} else {
					$product_image = '';
				}
		...
		$data['continue'] = $this->url->link('account/edit', 'language=' . $this->config->get('config_language') . '&customer_token=' . $this->session->data['customer_token']);

		$data['form_url'] = $this->url->link('account/order', 'language=' . $this->config->get('config_language') . '&customer_token=' . $this->session->data['customer_token']);
