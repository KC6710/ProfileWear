...
class Manufacturer extends \Opencart\System\Engine\Controller {
	...
	public function info(): void {
		...
		// echo "<pre>"; print_r($manufacturer_info); die;
		...
		if ($manufacturer_info) {
			...
			if($manufacturer_info['background_image']){
				if (is_file(DIR_IMAGE . html_entity_decode($manufacturer_info['background_image'], ENT_QUOTES, 'UTF-8'))) {
					$data['thumb'] = $this->model_tool_image->resize(html_entity_decode($manufacturer_info['background_image'], ENT_QUOTES, 'UTF-8'), $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
				} else {
					$data['thumb'] = '';
				}
			}

			$data['background_color'] = $manufacturer_info['color'];

			$data['manufacturerDescription'] = html_entity_decode($manufacturer_info['manufacturerDescription'], ENT_QUOTES, 'UTF-8');
