...
class Address extends \Opencart\System\Engine\Model {
	...
	function markDefaultAddress($addressId) {
		// Unmark all the addresses as the non-default address
			$this->db->query("UPDATE `" . DB_PREFIX . "address` SET `default` = '0' WHERE `customer_id` = '" . (int)$this->customer->getId() . "'");
	
		// Mark the new address as the default address
		$this->db->query("UPDATE `" . DB_PREFIX . "address` SET `default` = '1' WHERE `address_id` = '" . $addressId . "'");
	  }
}
