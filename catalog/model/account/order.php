...
class Order extends \Opencart\System\Engine\Model {
	...
	public function getOrders(int $start = 0, int $limit = 20, $search=''): array {
		if ($start < 0) {
			$start = 0;
		}
		...
		if(!$search) {
			...
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE `customer_id` = '" . (int)$this->customer->getId() . "' AND `order_status_id` > '0' AND `store_id` = '" . (int)$this->config->get('config_store_id') . "' AND `" . DB_PREFIX . "order`.`order_id` LIKE '%" . (int)$search . "%' ORDER BY `order_id` DESC LIMIT " . (int)$start . "," . (int)$limit);
		}
	...
	public function getTotalOrders($search=''): int {
		if(!$search) {
			...
			$query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "order` o WHERE `customer_id` = '" . (int)$this->customer->getId() . "' AND o.`order_status_id` > '0' AND o.`store_id` = '" . (int)$this->config->get('config_store_id') . "' AND o.`order_id` LIKE '%" . (int)$search . "%'");
		}
