...
class Filter {
	...
	public function users() {
		...
		if( ocme()->oc()->registry()->get('customer') && ocme()->oc()->registry()->get('customer')->isLogged() ) {
			$users[] = 'customer';
		}
		
		if( ocme()->oc()->registry()->get('user') && ocme()->oc()->registry()->get('user')->isLogged() ) {
			$users[] = 'user';
		}
	...
	public function getLanguageDirection() {
		return ocme()->oc()->registry()->get('language')->get('direction') == 'ltr' ? 'ltr' : 'rtl';
	}
	
	protected static function determinePosition( $position ) {
		if( 'auto' == $position ) {
			$position = ocme()->oc()->registry()->get('language')->get('direction') == 'ltr' ? 'left' : 'right';
		}
	...
	public function getEvents() {
		...
		foreach( array( 'before_mount', 'mounted', 'before_render', 'after_render' ) as $trigger ) {
			foreach( array( 'config', 'filter' ) as $type ) {
				/* @var $global_code string */
				if( null != ( $global_code = ocme()->variable()->get($type . '_global_js_hook.' . $trigger) ) ) {
	...
	public function mobileOpenButtonLabel( $default ) {
		...
		if( null != ( $labels = (array) $this->getConfig('widget.button.open.text') ) ) {
			if( ! empty( $labels[ocme()->oc()->registry()->get('config')->get('config_language_id')] ) ) {
				return $labels[ocme()->oc()->registry()->get('config')->get('config_language_id')];
			}
		}
