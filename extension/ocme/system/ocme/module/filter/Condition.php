...
class Condition {
	...
	public function __construct( Filter $filter, \Ocme\ModuleSetting $setting, array $config, array $data = array() ) {
		foreach( ocme()->arr()->dot( $setting->get( 'conditions.setting' ) ) as $key => $val ) {
			if( ocme()->arr()->get( $config, 'setting.'.$key ) === null ) {
				ocme()->arr()->set( $config, 'setting.'.$key, $val );
			}
		}
	...
	public function customCss() {
		...
		if( strpos( (string)$this->getConfig('setting.wrap_classes'), '-slack' ) ) {
	...
	public function getDataAsVue() {
		return $this->data ? ocme()->arr()->getAsVue( $this->data ) : base64_encode('{}');
	}
