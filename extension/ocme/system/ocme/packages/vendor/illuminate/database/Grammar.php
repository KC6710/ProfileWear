...
abstract class Grammar {
	...
	public function wrap($value, $prefixAlias = false)
	{
		...
		if (strpos(strtolower($value), ' as ') !== false)
		{
			...
			if( ocme()->str()->startsWith($segments[2], '`') && ocme()->str()->startsWith($segments[2], '`') ) {
		...
		foreach ($segments as $key => $segment)
		{
			...
			{
				$wrapped[] = ocme()->str()->startsWith($segment, '`') && ocme()->str()->startsWith($segment, '`') ? $segment : $this->wrapTable($segment);
			}
			else
			{
