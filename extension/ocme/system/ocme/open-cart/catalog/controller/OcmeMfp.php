...
trait OcmeMfp {
	...
	public function eventViewBefore($type, &$data) {
		...
		if( in_array( $type, \Ocme\Module\Filter::supportedRoutes() ) ) {
			...
			if( ocme()->variable()->get('config_global.products_wrapper.insert') == 'between_content_top_and_content_bottom' ) {
				$data['content_top'] .= sprintf( '<div id="%s" class="ocme-mfp-f-main-container %s">', ocme()->variable()->get('config_global.products_wrapper.id'), ocme()->variable()->get('config_global.products_wrapper.extra_class') );
