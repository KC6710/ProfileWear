...
trait OcmeMfp {	
	...
	protected function checkInstallation() {
		if( ocme()->oc()->registry()->get('config')->get('ocme_mfp_installation_error') ) {
			/* @var $errors array */
			$errors = array();
			
			/* @var $ocme_mfp_db_changes array */
			if( null != ( $ocme_mfp_db_changes = (array) ocme()->oc()->registry()->get('config')->get('ocme_mfp_db_changes') ) ) {
				/* @var $changes array */
		...
		if( !$version && ocme()->oc()->registry()->get('config')->get('ocme_mfp_installed_at') ) {
			$version = '3.0.0.0';
		...
		if( $this->isInstalledModule( 'ocme_mfp_filter') && ! ocme()->oc()->registry()->get('config')->get('ocme_mfp_installed_at') ) {
	...
	protected function rmCacheDirs( $dir ) {
		if( is_dir( $dir ) && is_writable( $dir ) ) {
			/* @var $items array */
			if( null != ( $items = glob( $dir . '/*' ) ) ) {
				/* @var $item string */
				foreach( $items as $item ) {
					if( is_dir( $item ) && is_writable( $item ) ) {
						$this->rmCacheDirs( $item );
