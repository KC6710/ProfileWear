...
class App extends \Illuminate\Container\Container {
	...
	public function isInstalled() {
		return $this->ocRegistry()->get('config')->get('ocme_mfp_installed_at');
	}
	
	public function environment() {
		if( version_compare( VERSION, '4', '>=' ) ) {
			return $this->ocRegistry()->get('config')->get('application');
		}
	...
	public function model( $name ) {
		if( strpos( $name, '/' ) !== false ) {
			$this->ocRegistry()->get('load')->model( $name );
			
			return $this->ocRegistry()->get( 'model_' . str_replace( '/', '_', $name ) );
		}
	...
	public function trans( $key = null, array $replace = array() ) {
		if( is_null( $key ) ) {
			return $this->ocRegistry()->get('language');
		}
		...
		if( strpos( $key, ' ' ) === false ) {
			if( strpos( $key, '::' ) !== false ) {
				...
				$this->ocRegistry()->get('load')->language( $key, $prefix );

				if( version_compare( VERSION, '4', '>=' ) ) {
					$trans = $this->ocRegistry()->get('language')->all( $prefix );
				} else {
					$trans = $this->ocRegistry()->get('language')->get( $prefix )->all();
				}
			} else {
				/* @var $trans string */
				$trans = $this->ocRegistry()->get('language')->get( $key );
			}
		} else {
			$trans = $key;
		}
