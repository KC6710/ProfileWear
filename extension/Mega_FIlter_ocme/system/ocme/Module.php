...
class Module {
	...
	public static function validStatus( $status ) {
		switch( $status ) {
			case 'customers' : {
				return ocme()->ocRegistry()->get('customer')->isLogged() || ocme()->ocRegistry()->get('user')->isLogged();
			}
			case 'guests' : {
				return ! ocme()->ocRegistry()->get('customer')->isLogged() && ! ocme()->ocRegistry()->get('user')->isLogged();
			}
			case 'admin' : {
				return (bool) ocme()->ocRegistry()->get('user')->isLogged();
			}
		}
	...
	public static function validStore( $stores ) {
		return $stores && in_array( ocme()->ocRegistry()->get('config')->get('config_store_id'), $stores );
	}
	...
	public static function validCustomerGroup( $customer_groups ) {
		...
		return in_array( 
			ocme()->ocRegistry()->get('customer')->isLogged() ? ocme()->ocRegistry()->get('customer')->getGroupId() : ocme()->ocRegistry()->get('config')->get('config_customer_group_id'), 
			$customer_groups 
		);
	}
