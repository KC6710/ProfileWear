...
class Filter {
	...
	public function __construct() {
		$this->config_customer_group_id = (int) ( ocme()->ocRegistry()->get('customer')->isLogged() ? ocme()->ocRegistry()->get('customer')->getGroupId() : ocme()->ocRegistry()->get('config')->get('config_customer_group_id') );
		$this->config_language_id = (int) ocme()->ocRegistry()->get('config')->get('config_language_id');
		$this->config_store_id = (int) ocme()->ocRegistry()->get('config')->get('config_store_id');
		$this->config_tax = (bool) ocme()->ocRegistry()->get('config')->get('config_tax');
	...
	public function paginationData() {		
		...
		/* @var $limit int */
		if( null == ( $limit = (int) ocme()->request()->query('limit', ocme()->ocRegistry()->get('config')->get('theme_' . ocme()->ocRegistry()->get('config')->get('config_theme') . '_product_limit')) ) ) {
			$limit = (int) ocme()->ocRegistry()->get('config')->get('config_pagination');
		}
	...
	public function pagination( array $url_parameters = array() ) {
		...
		if( version_compare( VERSION, '4', '>=' ) ) {
			$pagination_html = ocme()->ocRegistry()->get('load')->controller('common/pagination', array(
				'total' => $total,
				'page'  => $page,
				'limit' => $limit,
				'url'   => ocme()->ocRegistry()->get('url')->link( $route, implode( '&', $url ) . '&page={page}')
			));
		} else {		
			...
			$pagination->url = ocme()->ocRegistry()->get('url')->link( $route, implode( '&', $url ) . '&page={page}');
	...
	public function prepareConditions( ModuleSetting $setting ) {
		...
		$conditions = $this->getConditionsQuery( $setting )
			...
			->whereIn('status', OcmeFilterCondition::getStatuses())
			->get();
	...
	public function prepareExcludeConditions( ModuleSetting $setting ) {		
		/* @var $conditions \Illuminate\Database\Eloquent\Collection */
		$conditions = $this->getConditionsQuery( $setting )->whereIn('condition_type', array(
			OcmeFilterCondition::CONDITION_TYPE_ATTRIBUTE,
			OcmeFilterCondition::CONDITION_TYPE_ATTRIBUTE_GROUP,
			OcmeFilterCondition::CONDITION_TYPE_OPTION,
			OcmeFilterCondition::CONDITION_TYPE_FILTER_GROUP,
		))->whereNotIn('status', OcmeFilterCondition::getStatuses())->get();
				...
				switch( ocme()->arr()->get( $condition, 'name' ) ) {
					case 'price' : {
						if( ocme()->arr()->get( $condition, 'setting.show_currency' ) ) {
							/* @var $currency string */
							if( null != ( $currency = ocme()->arr()->get( ocme()->ocRegistry()->get('session')->data, 'currency' ) ) ) {
								ocme()->arr()->set( $condition, 'data.symbol_left', ocme()->ocRegistry()->get('currency')->getSymbolLeft( $currency ) );
								ocme()->arr()->set( $condition, 'data.symbol_right', ocme()->ocRegistry()->get('currency')->getSymbolRight( $currency ) );
							}
						}
	...
	public function getCurrencyValue() {
		return ocme()->ocRegistry()->get('currency')->getValue( ocme()->arr()->get( ocme()->ocRegistry()->get('session')->data, 'currency' ) );
	}
	...
	protected function applyAvailabilityCondition( $item, \Illuminate\Database\Eloquent\Builder $query, $postfix = '' ) {
		...
		if( $item instanceof Condition ) {
			if( $item->getModuleSetting()->get('configuration.other.only_available') == '1' ) {
			...
			if( $item->getConfig('vtype') == OcmeFilterCondition::CONDITION_TYPE_OPTION ) {
				...
				} else if( $item->getConfig('setting.only_available') == '1' ) {
					$query->where('`pov`.quantity', '>', 0);
				}
			}
	...
	protected function prepareValues( Condition $condition, \Illuminate\Database\Eloquent\Builder $query, array $where_conditions = array(), array $options = array() ) {		
		...
		$this
			...
			->applyAvailabilityCondition( $condition, $query )
		...
		$total_values = ocme()->arr()->get( 
			ocme()->db()->connection()->selectOne( sprintf( 'SELECT COUNT(*) AS `total` FROM( %s ) AS `tmp`', ocme()->db()->queryToRawSql( $query ) ) ),
			'total'
		);
		...
		if( $total_values ) {
			...
			foreach( $query->get() as $row_value ) {
				/* @var $value array */
	...
	protected function prepareValuesWithConditions( array & $items, Condition $condition, \Illuminate\Database\Eloquent\Builder $query, array $options = array(), $postfix = '' ) {
		...
		$this
			...
			->applyAvailabilityCondition( $condition, $query_with_conditions, $postfix );
		...
		if( 
			...
			if( null != ( $ids = array_fetch( $items, 'id' ) ) ) {
				...
				/* @var $item \stdClass */
				foreach( $query_with_conditions->get() as $item ) {
	...
	protected function createValue( Condition $condition, $row ) {
		...
		if( $condition->getConfig('condition_type') == OcmeFilterCondition::CONDITION_TYPE_BASE_ATTRIBUTE && $condition->getConfig('name') == 'availability' ) {
			if( in_array( ocme()->arr()->get( $value, 'id' ), array( 'in_stock', 'out_of_stock' ) ) ) {
				...
				if( '' != ( $custom_label = $condition->getConfig( sprintf('setting.%s_text.%s', ocme()->arr()->get( $value, 'id' ), ocme()->ocRegistry()->get('config')->get('config_language_id') ) ) ) ) {
	...
	public function applyCurrentConditionsIfPossible( Condition $condition, $query ) {
		...
		if( $this->shouldUseCurrentPostifx() ) {
			return $this->applyConditions( $query, $type, 'current', array(
				'vtype' => $condition->getConfig('vtype'),
				'vid' => $condition->getConfig('vid'),
			))->applyAvailabilityCondition( $condition, $query, 'current' );
		}
	...
	public function getTotalProducts() {
		...
		/* @var $query \Illuminate\Database\Eloquent\Builder */
		$query = $this->createProductsQuery( $postfix )
		...
		$this
			...
			->applyAvailabilityCondition( null, $query, $postfix );
	...
	public function getProducts() {
		...
		return $query->get()->map( function( $product ){
			return ocme()->ocRegistry()->get('model_catalog_product')->getProduct( $product->product_id );
		});
	}
	 ...
	 * @param type $type
	 * @param type $postfix
	 * @param type $force_filter_category
	 * @return $this
	 */
...
//				}
		...
		if( ! empty( $filter_name ) || ! empty( $filter_tag ) ) {
			...
			if( ocme()->arr()->first( (array) $joins, function( $join ){ return strpos( ocme()->arr()->get( $join, 'table', '' ), 'product_description' ) !== false; } ) ) {
				...
				$this->applySearchConditions2( $query, $type, '', array(
	...
	protected function applyOptionConditions( $query, $type = null, $postfix = '', array $options = array() ) {
		...
		$this
			->applyConditionSubQueryOptionIds(function( $option_id, $option_value_ids, $boolean ) use( $query, $quantity_range ){
				$this->applyWhereExists( $query, function($q, $option_value_ids) use( $option_id, $quantity_range ){
					$q
						->select( ocme()->db()->raw(1) )
...
//					}
					...
					if( $quantity_range && count( $quantity_range ) >= 2 ) {
	...
	protected function applySearchConditions2( $query, $type = null, $postfix = '', array $options = array() ) {
		if( $type != 'search' ) {
			/* @var $filter_name string */
			if( null === ( $filter_name = ocme()->arr()->get( $options, 'filter_name' ) ) ) {
				/* @var $search array */
				if( null != ( $search = ocme()->request()->conditionData( 'text', 'search', $this->requestNamespace( $postfix ) ) ) ) {
...
//		}
	...
	protected function applySortOrder( $query, $sorts, $sort, $order ) {
		...
		$query->orderBy( ocme()->db()->raw( 'LCASE(`pd`.`name`)' ), $order );
	...
	protected function applyTaxConditions( $query ) {
		...
		$conditions = array(
			'store' => array(
				'country_id' => (int) ocme()->ocRegistry()->get('config')->get('config_country_id'),
				'zone_id' => (int) ocme()->ocRegistry()->get('config')->get('config_zone_id'),
				'types' => array(),
			)
		);
		...
		foreach( array( 'payment', 'shipping' ) as $type ) {
			...
			foreach( array( 'country_id', 'zone_id' ) as $key ) {
				if( null == ( $conditions[$type][$key] = (int) ocme()->arr()->get( ocme()->ocRegistry()->get('session')->data, $type . '_' . $key ) ) ) {
