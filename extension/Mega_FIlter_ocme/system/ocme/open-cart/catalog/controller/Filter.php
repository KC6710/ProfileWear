...
trait Filter {
	...
	protected function jsDynamicContent() {
		...
		$js[] = "window.addEventListener('load', function() {";
			...
			$js[] = "ocmeFramework.extension('config').set(" . json_encode(array(
				'filter' => array(
					'url' => $this->url->link(ocme_extension_path('extension/module/ocme_mfp_filter'), '', true) . ( version_compare( VERSION, '4', '>=' ) ? '|' : '/' ),
		...
		if( ! self::$initialized ) {
			...
			$data = array_replace( $data, array(
				'fdata' => ocme()->arr()->getAsJson(array(
					...
					'redirect_url' => in_array( ocme()->request()->ocQueryRoute(), FilterModule::supportedRoutes() ) ? null : ocme()->ocRegistry()->get('url')->link('browse/catalog'),
				)),
				'global_custom_styles' => html_entity_decode( ocme()->variable()->get('filter_global_styles.custom_styles', ''), ENT_QUOTES, 'UTF-8'),
			));
		}
				...
				if( null != ( $oc_module = OcModule::where('code', OcModule::codeByOcVersion( OcModule::CODE_FILTER ))->find( $module_id ) ) ) {
					$this
						...
						->initializeFilterData(array(
							...
							'ocmef_only_availability' => ocme()->arr()->get($module_data, $module_id . '.only_availability', 0),
					...
					if( $load_results_via_ajax && null != ( $ocmef_source_route = ocme()->arr()->get( ocme()->ocRegistry()->get('request')->get, 'ocmef_source_route' ) ) ) {
						// Sanitize the call
		...
		if( '' !== ( $url_query = ocme()->request()->query( 'ocmef_url_query' ) ) ) {
			if( null != ( $url_query = base64_decode( $url_query ) ) ) {
				...
				if( $url_query_array ) {
					...
					ocme()->ocRegistry()->get('request')->get = array_replace( ocme()->ocRegistry()->get('request')->get, $url_query_array, $data );
				}
			}
		}
	...
	protected function isAllowedOrigin(&$route, &$args, $x = false) {
		...
		if( null != ( $prev_controller = array_shift( $backtrace ) ) ) {			
			if( strpos( $prev_controller, get_class( $this ) ) === 0 || strpos( $prev_controller, 'Ocme\\OpenCart\\Catalog\\Controller' ) === 0 ) {
		...
		if( ! in_array( $prev_controller, array(
			...
			'ControllerProductManufacturer::index',
	...
	public function eventGetProduct( &$route, &$args, &$data ) {
		...
		/* @var $config_tax bool */
		$config_tax = ocme()->ocRegistry()->get('config')->get('config_tax');
		
		if( $config_tax ) {
			$price_with_tax = ocme()->ocRegistry()->get('tax')->calculate($price_with_tax, $tax_class_id, $config_tax);
		}
		
		/* @var $sql string */
		$sql = "IF(`pov`.`price_prefix` = '+', `pov`.`price`, -`pov`.`price`)";

		if( ocme()->ocRegistry()->get('config')->get('config_tax') ) {
