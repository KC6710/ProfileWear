...
trait OcmeMfp {
	...
	public function connect() {		
		...
		if( version_compare( VERSION, '4', '>=' ) ) {
			if( ocme()->ocRegistry()->get('config')->get('config_session_samesite') != 'None' ) {
				...
				$this->data['text_connect_not_available'] = ocme()->trans('module::global.text_connect_not_available', array(
					'samesite' => ocme()->ocRegistry()->get('config')->get('config_session_samesite'),
				));
			}
		}
		...
		if( $type == 'after' ) {
			if( ocme()->oc()->isV4() ) {
				if( in_array( $route, array( 'catalog/attribute_list', 'catalog/attribute_group_list' ) ) ) {
					$operations['<div class="table-responsive">'] = '<div id="ocme-app" class="table-responsive">';
				} else if( in_array( $route, array( 'catalog/attribute_form' ) ) ) {
					$operations['<div class="card">'] = '<div id="ocme-app" class="card">';
				}
			} else if( in_array( $route, array( 'catalog/attribute_form', 'catalog/attribute_list', 'catalog/attribute_group_list' ) ) ) {
				$operations['<div class="panel panel-default">'] = '<div id="ocme-app" class="panel panel-default">';
			}
		}
		
		switch( true ) {
			case $type == 'after' && $route == 'catalog/attribute_form' : {
				/* @var $attribute Attribute */
				$attribute = Attribute::firstOrNew(array(
					'attribute_id' => ocme()->request()->query('attribute_id')
				));
				
				$operations['<form'] = sprintf(
					'<ocme-attribute-form form_action="%s" :oc="%s" :attribute="%s" :descriptions="%s" attribute_group_id="%s" :config="%s"></ocme-attribute-form><!-- <form',
					ocme()->url()->adminLink( ocme()->oc()->isV4() ? 'catalog/attribute|save' : ocme()->request()->ocQueryRoute(), ocme()->arr()->except( ocme()->request()->query(), 'route' ) ),
					ocme()->oc()->toJS(),
					ocme()->arr()->getAsVue( $attribute->toArray() ),
					ocme()->arr()->getAsVue( $attribute->descriptions->toArray() ),
					ocme()->request()->post('attribute_group_id', $attribute->attribute_group_id),
					ocme()->variable()->getAsVue('attribute')
				);
			...
			case $type == 'after' && $route == 'catalog/product_form' : {
				$tab_ocme = '<div class="tab-pane ocme ocme-bootstrap" id="tab-attribute-ocme"><div id="ocme-app"><ocme-product-attributes license="' . base64_encode(json_encode(array_replace( ocme()->license()->shopData(), array(
					'token' => ocme()->license()->token(),
					'version' => ocme()->version(),
				)))) . '" displayed_values_separator="' . ocme()->variable()->store(null)->get('attribute.displayed_values_separator') . '" /></div></div><div class="tab-pane" id="tab-attribute">';
			...
			case $type == 'before' && $route == 'common/column_left' : {				
				if( ocme()->ocRegistry()->get('user') && ocme()->ocRegistry()->get('user')->hasPermission('access', ocme_extension_path('extension/module/ocme_mfp') ) ) {
					if( ! isset( $data['menus'] ) ) {
						$data['menus'] = array();
					}
