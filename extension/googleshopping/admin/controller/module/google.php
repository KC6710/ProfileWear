...
namespace Opencart\Admin\Controller\Extension\Googleshopping\Module;
//for 4.0.0.0
require_once DIR_EXTENSION . '/googleshopping/system/library/library_autoloader.php';

use \Opencart\System\Library\Extension\Googleshopping\ConnectionException;
use \Opencart\System\Library\Extension\Googleshopping\Googleshopping;
use \Opencart\System\Library\Extension\Googleshopping\LibraryloaderTrait;
use \Opencart\System\Library\Extension\Googleshopping\StoreloaderTrait;

class Google extends \Opencart\System\Engine\Controller {
    use StoreloaderTrait;
    use LibraryloaderTrait;
    ...
    public function __construct(\Opencart\System\Engine\Registry $registry) {
        ...
        $this->config->addPath('extension/googleshopping', DIR_EXTENSION . 'googleshopping/system/config');

        $this->store_id = isset($this->request->get['store_id']) ? (int)$this->request->get['store_id'] : 0;

        $this->load->model('extension/googleshopping/proxy/url');
    ...
    public function index(): void {
        $this->load->language('extension/googleshopping/module/google');

        $this->load->model('extension/googleshopping/module/google');

        $this->load->config('extension/googleshopping/googleshopping');

        // Fix clashes with third-party extension table names
        $this->model_extension_googleshopping_module_google->renameTables();
        ...
        $this->model_extension_googleshopping_module_google->createTables();

        // Fix a missing AUTO_INCREMENT
        $this->model_extension_googleshopping_module_google->fixColumns();
        
        // Redirect to the preliminary check-list
        if (!$this->setting->get('advertise_google_checklist_confirmed')) {
            $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|checklist', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
        }

        try {
            // If we have not connected, navigate to connect screen
            if (!$this->setting->has('advertise_google_access_token')) {
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|connect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
                ...
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|merchant', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
                ...
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
                ...
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|shipping_taxes', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
            } else if (count($this->model_extension_googleshopping_module_google->getMapping($this->store_id)) == 0) {
                // In case the merchant has not set up mapping, redirect them to the form for mapping
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|mapping', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
            }
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateSettings()) {
            ...
            try {
                ...
                $this->response->redirect($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true));
            } catch (ConnectionException $e) {
                $this->session->data['error'] = $e->getMessage();
        ...
        $advertised_count = $this->model_extension_googleshopping_module_google->getAdvertisedCount($this->store_id);
        $last_cron_executed = (int)$this->setting->get('advertise_google_cron_last_executed');

        $data['warning'] = '';

        if (!$this->setting->get('advertise_google_status') && $this->model_extension_googleshopping_module_google->hasActiveTarget($this->store_id)) {
            $data['warning'] = $this->language->get('warning_disabled');
        } else if (!$this->model_extension_googleshopping_module_google->hasActiveTarget($this->store_id)) {
            $data['warning'] = sprintf($this->language->get('warning_no_active_campaigns'), $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true));
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->model_extension_googleshopping_proxy_url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'], true),
        );
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_extensions'),
            'href' => $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true),
        );
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true),
        );
        ...
        $data['cancel'] = $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['action'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        $data['shipping_taxes'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|shipping_taxes', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true);
        $data['campaign'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true);
        $data['mapping'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|mapping', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true);
        $data['disconnect'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|disconnect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        $data['list_ads'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|list_ads', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['advertise'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|advertise', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_popup'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|popup_product', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_category_autocomplete'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|category_autocomplete', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_debug_log_download'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|debug_log_download', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        ...
        if (VERSION > '4.0.1.1') {
            $data['advertise_google_cron_url'] = 'https://' . rtrim($host_and_uri, '/') . '/index.php?route=extension/googleshopping/module/google.cron&cron_token={CRON_TOKEN}';
        } else {
            $data['advertise_google_cron_url'] = 'https://' . rtrim($host_and_uri, '/') . '/index.php?route=extension/googleshopping/module/google|cron&cron_token={CRON_TOKEN}';
        }
        ...
        $data['product_autocomplete_url'] = $this->model_extension_googleshopping_proxy_url->link('catalog/product|autocomplete', 'user_token=' . $this->session->data['user_token'], true);

        $data['tab_settings'] = $this->load->view('extension/googleshopping/module/google_settings', $data);
        $data['tab_ads']      = $this->load->view('extension/googleshopping/module/google_ads', $data);
        $data['tab_reports']  = $this->load->view('extension/googleshopping/module/google_reports', $data);

        $data['header']       = $this->load->controller('common/header');
        $data['column_left']  = $this->load->controller('common/column_left');
        $data['footer']       = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/googleshopping/module/google', $data));
    }
    ...
    public function advertise(): void {
        ...
        if ($this->validatePermission()) {
            ...
            if (!empty($select) || !empty($filter_data)) {
                ...
                if (!empty($select)) {
                    $this->model_extension_googleshopping_module_google->setAdvertisingBySelect($select, $target_ids, $this->store_id);
                } else if (!empty($filter_data)) {
                    $this->model_extension_googleshopping_module_google->setAdvertisingByFilter($filter_data, $target_ids, $this->store_id);
                }
        ...
        if (!$this->setting->get('advertise_google_status') && $this->model_extension_googleshopping_module_google->hasActiveTarget($this->store_id)) {
            ...
            $json['warning'] = sprintf($this->language->get('warning_no_active_campaigns'), $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true));
        } else if ($advertised_count == 0) {
    ...
    public function list_ads(): void {
        ...
        $this->model_extension_googleshopping_module_google->insertNewProducts(array(), $this->store_id);
        ...
        $pages = ceil($product_total / $this->config->get('config_pagination_admin'));

        $json['showing'] = sprintf($this->language->get('text_pagination'), ($product_total) ? (($page - 1) * $this->config->get('config_pagination_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_pagination_admin')) > ($product_total - $this->config->get('config_pagination_admin'))) ? $product_total : ((($page - 1) * $this->config->get('config_pagination_admin')) + $this->config->get('config_pagination_admin')), $product_total, $pages);
    ...
    public function merchant(): void {
        ...
        $this->document->addStyle(HTTP_CATALOG . 'extension/googleshopping/admin/view/stylesheet/googleshopping/stepper.css');

        $this->load->model('extension/googleshopping/module/google');

        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validatePermission()) {
            try {
                $redirect_uri = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|callback_merchant', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|merchant', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true),
        );

        $data['cancel']       = $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['action']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|merchant', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        ...
        $data['current_step'] = 2;
        $data['steps'] = $this->load->view('extension/googleshopping/module/google_steps', $data);

        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_merchant', $data));
    }
    ...
    public function shipping_taxes(): void {
        ...
        if ($data['from_dashboard']) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|shipping_taxes', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true),
            );
        }

        if ($data['from_dashboard']) {
            $data['cancel']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        } else {
            $data['cancel']       = $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        }

        $data['action']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|shipping_taxes', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_shipping_taxes', $data));
    }
    ...
    public function mapping(): void {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateMapping()) {
            try {
                foreach ($this->request->post['advertise_google_mapping'] as $google_product_category => $category_id) {
                    if ($category_id) {
                        $this->model_extension_googleshopping_module_google->setCategoryMapping($google_product_category, $this->store_id, $category_id);
                    } else {
                        $this->model_extension_googleshopping_module_google->setCategoryMapping($google_product_category, $this->store_id, 0);
                    }
                }

                if (!empty($this->request->post['advertise_google_modify_existing'])) {
                    $this->model_extension_googleshopping_module_google->updateGoogleProductCategoryMapping($this->store_id);
                }
        ...
        if ($data['from_dashboard']) {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_shipping_taxes'),
                'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|mapping', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true),
            );
        }
        ...
        foreach ($this->config->get('advertise_google_google_product_categories') as $google_product_category_id => $google_product_category_name) {
            ...
            if (null !== $category = $this->model_extension_googleshopping_module_google->getMappedCategory($google_product_category_id, $this->store_id)) {
        ...
        $data['action']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|mapping', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);

        $data['user_token'] = $this->session->data['user_token'];

        $data['url_mapping_verify'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|mapping_verify', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_category_autocomplete'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|category_autocomplete', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        ...
        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_mapping', $data));
    }
    ...
    public function mapping_verify(): void {
        ...
        $json = array(
            'submit_directly' => !$this->model_extension_googleshopping_module_google->isAnyProductCategoryModified($this->store_id),
            'modal_confirmation' => $this->load->view('extension/googleshopping/module/google_mapping_verify', $data)
        );
    ...
    public function campaign_test(): void {
        ...
        if ($this->validatePermission()) {
            try {
                ...
                $json['redirect'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|connect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
            } catch (\RuntimeException $e) {
                $json['status'] = false;
    ...
    public function campaign(): void {
        ...
        if ($data['from_dashboard']) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&from_dashboard=true', true),
            );
        }
        ...
        $data['action'] = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        $data['target_add'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|target_add', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['target_edit'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|target_edit', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&advertise_google_target_id={target_id}', true), ENT_QUOTES, 'UTF-8');
        $data['target_delete'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|target_delete', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&advertise_google_target_id={target_id}', true), ENT_QUOTES, 'UTF-8');
        $data['target_list'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|target_list', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        $data['url_campaign_test'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|campaign_test', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        ...
        $data['json_allowed_targets'] = json_encode($this->model_extension_googleshopping_module_google->getAllowedTargets());
        ...
        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_campaign', $data));
    }
    ...
    public function target_add(): void {
        ...
        if ($this->validatePermission()) {
            if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateTarget()) {
                ...
                $this->model_extension_googleshopping_module_google->addTarget($target, $this->store_id);
    ...
    public function connect(): void {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateSettings() && $this->validateConnect()) {
            ...
            $this->session->data['advertise_google']['redirect_uri'] = html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|callback_connect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true), ENT_QUOTES, 'UTF-8');
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|connect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true),
        );
        ...
        $data['cancel']       = $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['action']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|connect', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_connect', $data));
    }
    ...
    public function checklist(): void {
        ...
        $data['cancel']       = $this->model_extension_googleshopping_proxy_url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['action']       = $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|checklist', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'], true);
        
        $data['header']       = $this->load->controller('common/header');
        $data['column_left']  = $this->load->controller('common/column_left');
        $data['footer']       = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/googleshopping/module/google_checklist', $data));
    }
    ...
    public function popup_product(): void {
        ...
        $this->language->load('extension/googleshopping/module/google');
        ...
        if ($this->request->post['operand']['type'] == 'single') {
            ...
            $product_info = $this->model_extension_googleshopping_module_google->getProductByProductAdvertiseGoogleId($product_advertise_google_id);
            ...
            if ($product_info !== NULL) {
                ...
                $required_fields = $this->model_extension_googleshopping_module_google->getRequiredFieldsByProductIds(array($product_info['product_id']), $this->store_id);
                ...
                $options = $this->model_extension_googleshopping_module_google->getProductOptionsByProductIds(array($product_info['product_id']));

                $default_form_data = $this->model_extension_googleshopping_module_google->getProductAdvertiseGoogle($product_advertise_google_id);
            }
            ...
            if (!empty($this->request->post['operand']['data']['all_pages'])) {
                ...
                $required_fields = $this->model_extension_googleshopping_module_google->getRequiredFieldsByFilter($filter_data, $this->store_id);

                if ($this->request->post['action'] == 'submit') {
                    $form_data = $this->request->post['form'];
                }

                $options = $this->model_extension_googleshopping_module_google->getProductOptionsByFilter($filter_data);
            } else {
                ...
                $required_fields = $this->model_extension_googleshopping_module_google->getRequiredFieldsByProductIds($product_ids, $this->store_id);

                if ($this->request->post['action'] == 'submit') {
                    $form_data = $this->request->post['form'];
                }

                $options = $this->model_extension_googleshopping_module_google->getProductOptionsByProductIds($product_ids);
            }
        ...
        if ($operand_info !== NULL) {
            ...
            if ($this->request->post['action'] == 'submit' && $this->validateProduct($required_fields)) {
                ...
                if ($this->request->post['operand']['type'] == 'single') {
                    $this->model_extension_googleshopping_module_google->updateSingleProductFields($form_data);
                } else if ($this->request->post['operand']['type'] == 'multiple') {
                    if (!empty($this->request->post['operand']['data']['all_pages'])) {
                        $this->model_extension_googleshopping_module_google->updateMultipleProductFields($filter_data, $form_data);
                    } else {
            ...
            $json['body'] = $this->load->view('extension/googleshopping/module/google_popup_product', $data);
        } else {
    ...
    public function popup_issues(): void {
        ...
        $product_issues = $this->model_extension_googleshopping_module_google->getProductIssues($product_id, $this->store_id);
        ...
        if ($product_issues !== NULL) {
            ...
            $json['body'] = $this->load->view('extension/googleshopping/module/google_popup_issues', $data);
        } else {
            $json['title'] = $this->language->get('error_popup_not_found_title');
            $json['body'] = $this->language->get('error_popup_not_found_body');
        }
    ...
    public function admin_link(string &$route, array &$data): void {
        if (!$this->user->hasPermission('access', 'extension/googleshopping/module/google')) {
            return;
        }
        ...
        foreach ($data['menus'] as &$menu) {
            if ($menu['id'] == 'menu-marketing') {
                ...
                $children[] = array(
                    ...
                    'children' => array(),
                    'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=0&user_token=' . $this->session->data['user_token'], true)
                );
                ...
                foreach ($this->model_setting_store->getStores() as $store) {
                    $children[] = array(
                        'name' => $store['name'],
                        'children' => array(),
                        'href' => $this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google', 'store_id=' . $store['store_id'] . '&user_token=' . $this->session->data['user_token'], true)
                    );
                }
    ...
    public function addProduct(string &$route, array &$args, int &$output): void {
        $this->load->model('extension/googleshopping/module/google');
        $this->load->model('catalog/product');

        foreach ($this->model_catalog_product->getProductStores($output) as $store_id) {
            $this->model_extension_googleshopping_module_google->insertNewProducts(array($output), $store_id);
        }
    }
    ...
    public function copyProduct(string &$route, array &$args, mixed &$output): void {
        ...
        $final_product_id = $this->model_extension_googleshopping_module_google->getFinalProductId();

        if (!empty($final_product_id)) {
            foreach ($this->model_catalog_product->getProductStores($final_product_id) as $store_id) {
                $this->model_extension_googleshopping_module_google->insertNewProducts(array($final_product_id), $store_id);
            }
        }
    }
    ...
    public function deleteProduct(string &$route, array &$args, mixed &$output): void {
        ...
        $this->model_extension_googleshopping_module_google->deleteProducts(array((int)$args[0]));
    }
    ...
    public function install(): void {
        ...
        $this->model_extension_googleshopping_module_google->createTables();
        $this->model_extension_googleshopping_module_google->createEvents();
    }
    ...
    public function uninstall(): void {
        ...
        $this->model_extension_googleshopping_module_google->dropTables();
        $this->model_extension_googleshopping_module_google->deleteEvents();
    }
    ...
    public function category_autocomplete(): void {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_extension_googleshopping_module_google->getCategories($filter_data, $this->store_id);
    ...
    protected function product(array $row): array {        
        $this->load->config('extension/googleshopping/googleshopping');

        $this->load->model('tool/image');

        if (!empty($row['image']) && is_file(DIR_IMAGE . $row['image'])) {
        ...
        return array(
            ...
            'url_issues' => html_entity_decode($this->model_extension_googleshopping_proxy_url->link('extension/googleshopping/module/google|popup_issues', 'store_id=' . $this->store_id . '&user_token=' . $this->session->data['user_token'] . '&product_id=' . $row['product_id'], true), ENT_QUOTES, 'UTF-8'),
            'campaigns' => $this->model_extension_googleshopping_module_google->getProductCampaigns((int)$row['product_id'], $this->store_id)
        );
    }

    protected function getSettingValue(string $key, mixed $default = null, bool $checkbox = false): mixed {
        if ($checkbox) {
    ...
    protected function validateProduct(string $required_fields): bool {
        if (!$this->user->hasPermission('modify', 'extension/googleshopping/module/google')) {
    ...
    protected function validateConnect(): bool {
        ...
        } else if ($this->model_extension_googleshopping_module_google->isAppIdUsed($this->request->post['advertise_google_app_id'], $this->store_id)) {
