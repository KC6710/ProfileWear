...
use \Opencart\System\Library\Extension\Googleshopping\Library;
...
use \Opencart\System\Library\Extension\Googleshopping\AccessForbiddenException;
...
class Googleshopping extends Library {
    ...
    public function __construct($registry, $store_id) {
        ...
        if ($this->store_id === 0) {
            $this->store_url = defined('HTTP_CATALOG') ? HTTP_CATALOG : HTTP_SERVER;
            $this->store_name = $this->config->get('config_name');
        } else {
            $this->store_url = $this->model_setting_setting->getValue('config_url', $store_id);
            $this->store_name = $this->model_setting_setting->getValue('config_name', $store_id);
        }
    ...
    public function rewrite($link) {
        ...
        foreach ($data as $key => $value) {
            if (isset($data['route'])) {
                if (($data['route'] == 'product/category' && $key == 'path') || ($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'information/information' && $key == 'information_id') || ($key == 'language')) {
    ...
    protected function getFeedProducts($page, $language_id, $currency) {
        ...
        foreach ($this->db->query($sql)->rows as $row) {
            ...
            $url = new \Opencart\System\Library\Url($this->store_url, $this->store_url);
        ...
        if ($this->getJobs()) {
            foreach ($this->getJobs() as $job) {
                try {
                    ...
                    $this->config->set("config_seo_url", $this->model_setting_setting->getValue("config_seo_url", $this->store_id));
    ...
    protected function resize($filename, $width, $height) {
        ...
        $this->load->model('extension/googleshopping/proxy/helper');

        $extension = pathinfo($filename, PATHINFO_EXTENSION);

        $image_old = $filename;
        $image_new = 'cache/' . $this->model_extension_googleshopping_proxy_helper->substr($filename, 0, $this->model_extension_googleshopping_proxy_helper->strrpos($filename, '.')) . '-' . (int)$width . 'x' . (int)$height . '.' . $extension;
    ...
    protected function sanitizeText($text, $limit) {
        $this->load->model('extension/googleshopping/proxy/helper');
        return $this->model_extension_googleshopping_proxy_helper->substr(
            trim(
                preg_replace(
                    '~\s+~', 
                    ' ', 
    ...
    public function convertAndFormat($price, $currency) {
        $currency_converter = new \Opencart\System\Library\Cart\Currency($this->registry);
    ...
    public function getCampaignReports() {
        ...
        $cache = new \Opencart\System\Library\Cache($this->config->get('cache_engine'), self::CACHE_CAMPAIGN_REPORT);
    ...
    public function getProductReports($product_ids) {
        $cache = new \Opencart\System\Library\Cache($this->config->get('cache_engine'), self::CACHE_PRODUCT_REPORT);
