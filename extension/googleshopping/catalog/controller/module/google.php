...
namespace Opencart\Catalog\Controller\Extension\Googleshopping\Module;
...
class Google extends \Opencart\System\Engine\Controller {
    ...
    public function google_global_site_tag(string &$route, array &$data, string &$output): void {
    ...
    public function google_dynamic_remarketing_purchase(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $purchase_snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_purchase', $data);
    ...
    public function google_dynamic_remarketing_home(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_home', $data);

        // Insert the snippet after the output
        $output = str_replace('</body>', $snippet . '</body>', $output);
    }

    public function google_dynamic_remarketing_searchresults(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_searchresults', $data);

        // Insert the snippet after the output
        $output = str_replace('</body>', $snippet . '</body>', $output);
    }

    public function google_dynamic_remarketing_category(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_category', $data);

        // Insert the snippet after the output
        $output = str_replace('</body>', $snippet . '</body>', $output);
    }

    public function google_dynamic_remarketing_product(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_product', $data);

        // Insert the snippet after the output
        $output = str_replace('</body>', $snippet . '</body>', $output);
    }

    public function google_dynamic_remarketing_cart(string &$route, array &$data, mixed &$output): void {
        // In case the extension is disabled, do nothing
        if (!$this->setting->get('advertise_google_status')) {
            return;
        }
        ...
        $snippet = $this->load->view('extension/googleshopping/module/google_dynamic_remarketing_cart', $data);

        // Insert the snippet after the output
        $output = str_replace('</body>', $snippet . '</body>', $output);
    }

    public function cron($cron_id = null, $code = null, $cycle = null, $date_added = null, $date_modified = null): void {
        $this->loadLibrary($this->store_id);
    ...
    protected function validateCRON(): bool {
        if (!$this->setting->get('advertise_google_status')) {
            // In case the extension is disabled, do nothing
            return false;
        }
