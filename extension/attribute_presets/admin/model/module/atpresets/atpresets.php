...
class Atpresets extends \Opencart\System\Engine\Model {
	...
	public function editPreset($preset_id, $data) {
		...
		if ($this->config->get('module_atpresets_autoupdate')==1) {
	...
	public function getPresetTitle($preset_id) {
		...
		if ($query->num_rows)
			return (strlen($query->row['text'])>50)?substr($query->row['text'], 0, 50).'...':$query->row['text'];
		else 
			return '';
	}
