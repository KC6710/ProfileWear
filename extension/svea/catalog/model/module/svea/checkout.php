...
class Checkout extends \Opencart\System\Engine\Model
{
    ...
    public function addOrder($order)
    {
        ...
        if (isset($order['products'])) {
            foreach ($order['products'] as $product) {
                $this->db->query("
                    INSERT INTO `" . DB_PREFIX . "codevoc_b2b_order_product` SET
                    `order_id`      = '" . (int)$order_id . "',
        ...
        $this->model_extension_opencart_total_voucher->disableVoucher($order_id);
        ...
        if (isset($order['totals'])) {
            foreach ($order['totals'] as $total) {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "order_total` SET
                    order_id    = '" . (int)$order_id . "',
		            extension   = '" . $this->db->escape($total['extension']) . "',
    ...
    public function updateOrder($order_id, $response)
    {
        ...
        if (isset($response['billingaddress']) && is_array($response['billingaddress'])) {
            ...
            if ($billing_country) {
                ...
                $data['payment_address_format'] = $this->db->escape($this->getAddressFormat($billing_country['address_format_id']));
            }
        ...
        if (isset($response['shippingaddress']) && is_array($response['shippingaddress'])) {
            ...
            if ($shipping_country) {
                ...
                $data['shipping_address_format'] = $this->db->escape($this->getAddressFormat($shipping_country['address_format_id']));
            }
        }
        ...
        if ($currentOpencartOrderStatus['order_status_id'] == 0 && $opencartOrderStatusIdFromSveaResponse == $this->config->get('config_order_status_id')) {
            $this->model_checkout_order->addHistory($order_id, $opencartOrderStatusIdFromSveaResponse, $comment, false);
        }
        return true;
    }
    ...
    public function getAddressFormat($address_format_id){
        if($address_format_id){
            $query = $this->db->query("SELECT * FROM `". DB_PREFIX ."address_format` WHERE `address_format_id`='".$address_format_id."'");
