$(document).ready(function () {
  ...
  continueBtnStep1.on('click', function () {
    ...
    if (postcode && postcode.length >= postCodeLength && postcode >= postCodeEl.val()) {
      ...
      $.ajax({
        type: 'POST',
        url: 'index.php?route=extension/svea/module/svea/shipping',
        data: $('#sco-form input'),
        dataType: 'json',
        success: function (json) {
          ...
          shippingEl.html(html);
            $.ajax({
                type: 'POST',
                url: 'index.php?route=extension/svea/module/svea/shipping.save',
                data: shippingEl,
                dataType: 'json',
                success: function () {
  ...
  $('#comment').on('blur', function (event) {
    ...
    $.ajax({
      type: 'POST',
      url: 'index.php?route=extension/svea/module/svea/comment',
      data: $('#comment'),
      dataType: 'json',
      success: function (json) {
        if ($('#comment').val() !== '') {
  ...
  function removeSpinner(addonType)
  {
    ...
    {
      let couponIconEl = $('#sco-coupon-button-icon');
      couponIconEl.addClass("fa fa-check");
      couponIconEl.removeClass('fa fa-spinner fa-spin');
    }
    else if(addonType === 'voucher')
    {
      let voucherIconEl = $('#sco-voucher-button-icon');
      voucherIconEl.addClass("fa fa-check");
      voucherIconEl.removeClass('fa fa-spinner fa-spin');
    }
    else if(addonType === 'comment')
    {
      let commentIconEl = $('#sco-comment-button-icon');
      commentIconEl.addClass("fa fa-check");
  ...
  $(document).on('click', '#sco-coupon-add', function () {
    addSpinner('coupon');
    $.ajax({
      url: 'index.php?route=extension/svea/module/svea/coupon.add',
      type: 'post',
      ...
      success: function (json) {
        if (json['error']) {
          ...
          couponEl.load('index.php?route=extension/svea/module/svea/coupon');
          updateCartInformation();
        }
      },
      complete: function() {
  ...
  $(document).on('click', '#sco-coupon-remove', function () {
    addSpinner('coupon');
    $.ajax({
      url: 'index.php?route=extension/svea/module/svea/coupon.remove',
      type: 'post',
      dataType: 'json',
      beforeSend: function () {
        $('.alert').remove();
  ...
  $(document).on('click', '#sco-voucher-add', function () {
    addSpinner('voucher');
    $.ajax({
      url: 'index.php?route=extension/svea/module/svea/voucher.add',
      type: 'post',
      ...
      success: function (json) {
        if (json['error']) {
          ...
          voucherEl.load('index.php?route=extension/svea/module/svea/voucher');
          updateCartInformation();
        }
      },
      complete: function() {
  ...
  $(document).on('click', '#sco-voucher-remove', function () {
    addSpinner('voucher');
    $.ajax({
      url: 'index.php?route=extension/svea/module/svea/voucher.remove',
      type: 'post',
      dataType: 'json',
      beforeSend: function () {
        $('.alert').remove();
  ...
  cartEl.load('index.php?route=extension/svea/module/svea/cart');
  couponEl.load('index.php?route=extension/svea/module/svea/coupon');
  voucherEl.load('index.php?route=extension/svea/module/svea/voucher');
  ...
  function initialCheckoutPayment(forceCreate) {
    ...
    var url = 'index.php?route=extension/svea/module/svea/payment';
    if (forceCreate) {
      url += '&create=true';
    }
