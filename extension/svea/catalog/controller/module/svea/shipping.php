    ...
    public function index()
    {
        ...
        foreach ($results as $result) {
            if ($this->config->get($this->shippingString .$result['code'] . '_status')) {
                if($result['extension'] != 'opencart'){
                    ...
                    $this->load->model('extension/opencart/shipping/' . $result['code']); 
                    $quote = $this->{'model_extension_opencart_shipping_' . $result['code']}->getQuote($address);
        ...
        foreach ($methods as $method) {
            foreach ($method['quote'] as $quote) {
                $name = (count($method['quote']) > 0) ? sprintf($text_multi, $method['title'], $quote['name']) : $method['title'];
