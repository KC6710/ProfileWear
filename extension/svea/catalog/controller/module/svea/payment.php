...
require_once(DIR_OPENCART . 'extension/svea/catalog/controller/module/svea/svea_common.php');
require_once(DIR_APPLICATION . '../svea/config/configInclude.php');
class Payment extends SveaCommon
{
    ...
    public function index()
    {
        ...
        if ($this->config->get($this->moduleString . 'sco_test_mode')) {
            $order_id = hash('crc32', HTTP_SERVER) . $order_id;
        }
        ...
        try {
            ...
            if (is_array($response) === true) {
                ...
                $this->session->data[$this->paymentString . 'svea_last_page'] = 'extension/svea/module/svea/payment';
            ...
            $this->response->setOutput($this->load->view('extension/svea/module/svea/payment', $data));
        } catch (\Exception $e) {
            header('HTTP/1.1 500 PHP Library Error');
            header('Content-Type: application/json; charset=UTF-8');
            $er = array();
    ...
    private function setOrderGeneralData($checkout_order_entry, $order_id)
    {
        ...
        if ($this->config->get($this->moduleString . 'sco_locale')) {
            ...
            $locale = strtoupper(array_key_exists('language',$this->session->data) ? $this->session->data['language'] : 'en-gb');
        }
        ...
        $checkout_order_entry
            ...
            ->setCheckoutUri($this->url->link('extension/svea/module/svea/checkout'))
            ->setConfirmationUri(str_replace('&amp;', '&', $this->url->link('extension/svea/module/svea/success' . '&order_id=' . $order_id . '&hash=' . hash('sha512', $order_id . $_COOKIE['OCSESSID']))))
            ->setPushUri(str_replace('&amp;', '&', urldecode($this->url->link('extension/svea/module/svea/push', array($this->paymentString . 'svea_order' => '{checkout.order.uri}')))))
    ...
    private function addOrder($order_id, $email)
    {
        ...
        $order = array(
            ...
            'shipping_code' 			=> isset($this->session->data['shipping_method']['shipping_code'])  	? $this->session->data['shipping_method']['shipping_code']  					: '',
            'comment' 					=> isset($this->session->data['comment']) 					? $this->session->data['comment'] 									: '',
            'affiliate_id' 				=> isset($affiliate_id) 									? $affiliate_id 													: '',
            'commission' 				=> isset($commission) 										? $commission 														: '',
            'marketing_id' 				=> isset($marketing_id) 									? $marketing_id 													: '',
            'tracking' 					=> isset($tracking) 										? $tracking 														: '',
        ...
        $order_id = $this->model_extension_svea_module_svea_checkout->addOrder($order);
        ...
        $this->model_extension_svea_module_svea_checkout->addCheckoutOrder($order_id, $locale);
