<?php
namespace Opencart\Admin\Controller\Extension\Svea\Module;
require_once(DIR_APPLICATION . '../svea/config/configInclude.php');

class Sco extends \Opencart\System\Engine\Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        // get language
        $this->load->language('extension/svea/module/sco');
        $this->load->language('extension/svea/module/svea_shared');
        $data = array();
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->load->model('extension/svea/module/svea/upgrade');
            $this->model_extension_svea_module_svea_upgrade->upgradeDatabase('sco');
        ...
        // Set action url
        $data['action'] = $this->url->link('extension/svea/module/sco', $this->userTokenString . 'token=' . $this->session->data[$this->userTokenString . 'token'], true);
        ...
        // Set custom events
        $this->load->model('extension/svea/module/svea/events');
        ...
        // set response
        $this->response->setOutput($this->load->view('extension/svea/module/sco', $data));
    }
    ...
    protected function validate()
    {
        $this->setVersionStrings();
        // Validate permission
        if (!$this->user->hasPermission('modify', 'extension/svea/module/sco')) {
    ...
    public function install()
    {
        ...
        $this->model_extension_svea_module_svea_events->addSveaCustomEvent(
            $this->moduleString . 'sco_edit_checkout_url' . $this->appendString,
            'catalog/controller/checkout/checkout/before',
            'extension/svea/module/svea/checkout.redirectToScoPage'
        );
        ...
        if (VERSION < 3.0) {
            if ($this->model_extension_event->getEvent($this->moduleString . 'sco_add_history_order_from_admin' . $this->appendString, "catalog/controller/api/order/history/before", "extension/svea/order/history") == null) {
                $this->model_extension_svea_module_svea_events->addSveaCustomEvent(
                    ...
                    'extension/svea/module/svea/order.history'
                );
            }
        } else {
    ...
    {
        $this->load->model('extension/svea/module/svea/campaigns');

        $result = $this->model_extension_svea_module_svea_campaigns->checkIfOrderScoTableExists();

        if (!$result->num_rows) {
            $this->model_extension_svea_module_svea_campaigns->createOrderScoTable();
        }
    }
