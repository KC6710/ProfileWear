<?php
namespace Opencart\Admin\Controller\Extension\Svea\Payment;
require_once(DIR_APPLICATION . '../svea/config/configInclude.php');

class Sveadirectbank extends \Opencart\System\Engine\Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/svea/payment/svea_directbank');
        $this->load->language('extension/svea/payment/svea_shared');
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/svea/payment/svea_directbank', $this->userTokenString . 'token=' . $this->session->data[$this->userTokenString . 'token'], true)
        );
        $data['action'] = $this->url->link('extension/svea/payment/svea_directbank', $this->userTokenString . 'token=' . $this->session->data[$this->userTokenString . 'token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/svea/payment/svea_directbank', $data));
    }

    private function validate()
    {
        if (!$this->user->hasPermission('modify', 'extension/svea/payment/svea_directbank')) {
